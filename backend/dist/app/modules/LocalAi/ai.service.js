...
function getTemplatesForCity(cityInfo, interests) {
    const baseTemplates = [
...
function estimateCostLabel(style) {
    if (!style)
        return "Moderate";
    const s = style.toLowerCase();
    if (s.includes("budget") || s.includes("low"))
        return "Low";
    if (s.includes("luxury") || s.includes("high"))
        return "High";
    return "Moderate";
}
