...
export default function TravelPlansTable({ travelPlans }: TravelPlansTableProps) {
  ...
  const handleUpdate = async (data: FormData) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text() || "Failed to update travel plan");
  ...
  const handleBlockUnblock = async (plan: TravelPlan) => {
    try {
      ...
      toast.success(plan.isActive ? "Travel plan blocked" : "Travel plan unblocked");
    } catch (err: any) {
      console.error(err);
    ...
    <>
          ...
          <tbody>
            {plansList.map((plan) => (
              <tr key={plan.id} className="border-t hover:bg-gray-50 transition">
                <td className="p-3">
                  <div className="flex items-center gap-3 min-w-[150px]">
                    ...
                    <div className="overflow-hidden">
                      ...
                      <p className="text-xs text-gray-500 truncate">{plan.destination}</p>
                    </div>
                  </div>
                </td>
                <td className="p-3">
                  <div className="flex items-center gap-2 min-w-[120px]">
                    ...
                    />
                    <span className="truncate">{plan.user?.fullName || "Unknown User"}</span>
                  </div>
                </td>
                <td className="p-3 text-center">
                  <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-700">{plan.travelType}</span>
                </td>
                <td className="p-3 text-center text-xs">
                  {new Date(plan.startDate).toLocaleDateString()}<br />–<br />
                  {new Date(plan.endDate).toLocaleDateString()}
                </td>
                <td className="p-3 text-center">৳{plan.minBudget} – ৳{plan.maxBudget}</td>
                <td className="p-3 text-center">
                  {plan.isActive ? (
                ...
                <td className="p-3 text-center text-xs">{new Date(plan.createdAt).toLocaleDateString()}</td>
                <td className="p-3 text-center flex justify-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className={`p-2 rounded ${plan.isActive ? "hover:bg-red-100 text-red-600" : "hover:bg-green-100 text-green-700"}`}
                  >
      ...
      {isOpen && selectedPlan && (
        <UpdateModal
          ...
          onClose={() => { setIsOpen(false); setSelectedPlan(null); }}
        />
      )}
    </>
  );
